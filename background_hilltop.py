...
lam_values = np.empty(3)
V0_values = np.empty(3)
with open('lambda_hilltop.txt','r') as archivo:
    lineas = archivo.readlines()
    for l,value in enumerate(lineas):
        if l>=0 and l<=2:
            lam_values[l] = np.float64(value)
        else:
            V0_values[l-3] = np.float64(value)
conver = 3.808339152e56 # from Mpl to Mpc^-1
conver_t = 3.700035326e42 # from Mpl to s^-1
...
for s in range(3):
    ...
    f_efolds_limit = lambda t: np.abs(np.abs(1/16*(phi_limit_appr)**2 + 1/(16*lam*phi_limit_appr**2) - 1/16*t**2 - 1/(16*lam*t**2))-67)
    ...
    if s == 0:
        background_sol = solve_ivp (background_ode, N, background_conditions, dense_output=True, method='BDF', rtol=5e-5, atol=1e-6, max_step=0.001) # To obtain evolution beyond eps=1: method='BDF', rtol=1e-9, atol=1e-10, max_step=0.001
    else:
        background_sol = solve_ivp (background_ode, N, background_conditions, dense_output=True, method='BDF', rtol=1e-4, atol=1e-5, max_step=0.01) # To obtain evolution beyond eps=1: method='BDF', rtol=1e-9, atol=1e-10, max_step=0.001
...
#     plt.figure(1+s)
#     line1 = plt.plot(phi(Ne_bey),phi_dot(Ne_bey),color='b',linewidth=2.5,label='Numerical')
#     line2 = plt.plot(x,y,linestyle='--',color='r',linewidth=2.5,label='de Sitter')
#     line3 = plt.plot(X,Y,linestyle='--',color='g',linewidth=2.5,label='Slow-roll limit')
...
#     name = 'phase_diagrama_beyond_{'+str(s+1)+'}.pdf'
...
#     plt.figure(4+s)
#     plt.plot(Ne_bey-Ne_star,phi(Ne_bey),linewidth=2.5,label='Numerical')
#     # plt.plot(XX,YY,linestyle='--',color='r',linewidth=2,label='de Sitter')
#     plt.plot(X,Y,linestyle='--',color='g',linewidth=2.5,label='Slow-roll limit')
...
#     plt.legend(prop=config2)
#     plt.tick_params(axis='both',which='major',labelsize=12.5)
#     name = 'evolution_field_{'+str(s+1)+'}.pdf'
#     # plt.savefig(name,bbox_inches='tight')
...
#     plt.tick_params(axis='both', which='major', labelsize=12.5)
#     name = 'Hubble_radius_{'+str(s+1)+'}.pdf'
#     # plt.savefig(name,bbox_inches='tight')
    ...
    fig, ax_main = plt.subplots(num=10+s)
    ...
    name = 'state_eq_{'+str(s+1)+'}.pdf'
    plt.savefig(name,bbox_inches='tight')

#     # ## Número de e-folds necesarios (prob.2)
#     # N_prob2 = 63.1233 + 1/4*np.log((V(phi(Ne_star)))**2/((8*np.pi)**2*energy_density(Ne_limit)))
#     # print('The number of e-folds necessaries are: '+str(N_prob2))
